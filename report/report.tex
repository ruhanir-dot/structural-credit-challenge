...
TSLA & 5.01\% & 7.75\% & 1.55 \\
XOM  & 0.80\% & 1.96\% & 2.46 \\
Ford & 6.46\% & 10.86\% & 1.68 \\
\bottomrule
\end{tabular}
\end{table}
\paragraph{High Coefficient of Variation:} 
All firms exhibit CV between 1.55 and 3.08, indicating that standard 
deviation is 1.6 to 3.1 times the mean PD. While no universal CV threshold 
...
(Ford, 6.46\% mean PD) and the lowest-risk firm (AAPL, 0.06\% mean PD) 
exhibit CV values substantially exceeding 1.0. This suggests an issue with the model 
structure rather than firm-specific calibration problems. Notably, AAPL produces the highest relative volatility ($CV = 3.08$) despite 
...
(March--April 2020, shaded region), with peak increases of 6--23 times their annual mean values. For example we see that Ford reaches 40\% PD (6.2× its mean), 
JPMorgan peaks at 32\% (8.5× its mean), and even low-risk AAPL spikes to 1.4\% (23× its mean).
...
20.5 percentage point single-day jump is almost 10 times this reference magnitude, while JPMorgan and Tesla show 7--8 percentage point jumps. 
...
relative and absolute instability. Low-risk firms (AAPL, XOM) suffer from high relative volatility (CV $>$ 2.4) where noise dominates signal whereas 
high-risk firms (Ford, JPM, TSLA) suffer from large absolute movements ranging from 7.7 to 20.5 percentage points. 
...
Even the best-performing case (XOM with CV = 2.46 and max jump = 2.5\%pts) exceeds our reference thresholds, 
...
\midrule
AAPL & 3.08 & 2.43 & $\downarrow$21.0\% & 0.008 & 0.005 & $\downarrow$35.3\% \\
JPM  & 2.24 & 1.91 & $\downarrow$15.0\% & 0.293 & 0.220 & $\downarrow$25.0\% \\
TSLA & 1.55 & 1.33 & $\downarrow$14.2\% & 0.406 & 0.208 & $\downarrow$48.7\% \\
XOM  & 2.46 & 2.06 & $\downarrow$16.2\% & 0.097 & 0.053 & $\downarrow$45.3\% \\
Ford & 1.68 & 1.41 & $\downarrow$16.2\% & 0.493 & 0.300 & $\downarrow$39.1\% \\
\midrule
\textbf{Avg} & \textbf{2.20} & \textbf{1.83} & $\bm{\downarrow}$\textbf{16.5\%} & \textbf{0.259} & \textbf{0.157} & $\bm{\downarrow}$\textbf{38.7\%} \\
\bottomrule
\end{tabular}
\end{table}

The improved model achieves substantial stability improvement across both our metrics. 
The average CV reduction of 16.5\%  demonstrates systematic improvement 
...
The average daily change reduction of 38.7\% represents dramatic operational improvement. 
...
The improvements demonstrate consistent patterns across all firms. We see CV reductions ranging from 14.2--21.0\%, with no firm showing less than 14\% improvement.
We also see daily change reductions ranging from 25.0--48.7\%, with all firms exceeding 25\% improvement. A case specific highlight is AAPL achieving the best CV reduction of 21.0\%,
demonstrating that smoothing is particularly effective for low-risk firms where baseline noise-to-signal ratios are highest due to small absolute PD values amplifying percentage variations. 
Another highlight is TSLA, achieving the best mean daily PD change reduction of 48.7\%, where smoothing cuts noise caused by high-frequency price movements
(often driven by news and sentiment characteristic of growth stocks) that often obscure underlying credit quality trends. All in all, 
...
quality improvements unfold over quarters, not weeks.
...
\subsubsection{Cross-Sectional Ranking Preservation}

\begin{table}[H]
\centering
\caption{Risk Ranking Preservation: Mean PD Comparison}
\label{tab:ranking_improved}
\begin{tabular}{lrrr}
\toprule
\textbf{Firm} & \textbf{Baseline Mean PD} & \textbf{Improved Mean PD} & \textbf{Change} \\
\midrule
Ford & 6.46\% & 6.58\% & +0.12\%pts \\
TSLA & 5.01\% & 5.23\% & +0.22\%pts \\
JPM  & 3.75\% & 3.77\% & +0.02\%pts \\
XOM  & 0.80\% & 0.79\% & $-$0.01\%pts \\
AAPL & 0.06\% & 0.06\% & 0.00\%pts \\
\bottomrule
\end{tabular}
\end{table}

Table 4 demonstrates that exponential smoothing preserves cross-sectional risk ranking.
The rank ordering remains identical in both models: Ford $>$ TSLA $>$ JPM $>$ XOM $>$ AAPL. 
The mean PD changes are minimal for all firms (within 0.22 percentage points), 
confirming that smoothing addresses time-series instability without distorting
the model's cross-sectional risk assessment. This preservation is crucial for risk management
applications where consistent firm rankings enable stable credit limits, portfolio allocation,
and pricing decisions across market conditions.
...
strengthen and cash flows recover. The 38.7\% 
...
debt data with forward-fill may miss sudden leverage changes during crises. Finally, smoothing effectiveness depends on noise-to-signal ratios; for firms 
experiencing genuine rapid credit changes, smoothing could dampen valid signals. Future 
work could address these through first-passage time models, stochastic volatility, or 
...
, and improves stability through exponential smoothing. The improved model achieves an average 16.5\% CV reduction 
and average 38.7\% daily change reduction while preserving cross-sectional risk ranking. The key insight we see
