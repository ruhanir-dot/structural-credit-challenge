...
\midrule
AAPL & 3.03 & 2.41 & $\downarrow$20.3\% & 0.012 & 0.008 & $\downarrow$33.8\% \\
JPM  & 2.22 & 1.89 & $\downarrow$14.9\% & 0.303 & 0.226 & $\downarrow$25.2\% \\
TSLA & 1.72 & 1.47 & $\downarrow$14.5\% & 0.320 & 0.161 & $\downarrow$49.7\% \\
XOM  & 2.52 & 2.10 & $\downarrow$16.5\% & 0.086 & 0.047 & $\downarrow$45.1\% \\
Ford & 1.69 & 1.42 & $\downarrow$16.2\% & 0.488 & 0.297 & $\downarrow$39.1\% \\
\midrule
\textbf{Avg} & \textbf{2.23} & \textbf{1.86} & $\bm{\downarrow}$\textbf{16.5\%} & \textbf{0.242} & \textbf{0.148} & $\bm{\downarrow}$\textbf{38.6\%} \\
\bottomrule
\end{tabular}
\end{table}
...
The average daily change reduction of 38.6\% represents dramatic operational improvement. 
...
The improvements demonstrate consistent patterns across all firms. We see CV reductions ranging from 14.5--20.3\%, with no firm showing less than 14\% improvement.
We also see daily change reductions ranging from 25.2--49.7\%, with all firms exceeding 25\% improvement. A case specific highlight is AAPL achieving the best CV reduction of 20.3\%,
demonstrating that smoothing is particularly effective for low-risk firms where baseline noise-to-signal ratios are highest. 
Another highlight is Tesla, achieving the best mean daily PD change reduction of 49.7\%, where smoothing cuts noise caused by high-frequency price movements
(often driven by news and sentiment which makes sense for a growth stock),  that often obscure underlying credit quality trends. All in all, 
the constency of improvement across all five firms exemplify that exponential smoothing provides systematic benefits regardless of baseline risk level.
...
\midrule
Ford & 6.38\% & 6.50\% & +0.12\%pts \\
JPM  & 3.90\% & 3.92\% & +0.02\%pts \\
TSLA & 3.53\% & 3.67\% & +0.14\%pts \\
XOM  & 0.70\% & 0.69\% & $-$0.01\%pts \\
AAPL & 0.10\% & 0.10\% & 0.00\%pts \\
\bottomrule
\end{tabular}
\end{table}

Table 4 shows us that exponential smoothing preserves cross-sectional risk ranking.
 The overall average PD for each firm changes minimally (within 0.2 percentage points for all firms), 
 and the rank ordering remains the same : Ford $>$ JPM $>$ TSLA $>$ XOM $>$ AAPL. 
 This confirms that the smoothing approach addresses time-series instability without distorting
  the model's cross-sectional risk assessment. 
...
strengthen and cash flows recover. The 38.6\% 
...
debt data with forward-fill may miss sudden leverage changes during crises. Finally, 
smoothing effectiveness depends on noise-to-signal ratios; for firms experiencing genuine 
...
and average 38.6\% daily change reduction while preserving cross-sectional risk ranking. The key insight we see
